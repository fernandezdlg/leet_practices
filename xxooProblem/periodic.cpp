...
{
public:
    size_t combinationsPeriodicBC(size_t N, bool xx, bool xlast) {
        // Rules:
        ...
        if (xx) {
            if (xlast) {
                // 11: 00, 01 allowed:
                return combinationsPeriodicBC(N - 2, false, false) + combinationsPeriodicBC(N - 2, true, true);
            } else {
                // 11..0 : 00, 01, 10 allowed:
                return combinationsPeriodicBC(N - 2, false, false) + combinationsPeriodicBC(N - 2, false, true) + combinationsPeriodicBC(N - 2, true, false);
            }
        } else if (xlast) {
            // 00..1: 10, 00, 01 allowed:
            return combinationsPeriodicBC(N - 2, true, false) + combinationsPeriodicBC(N - 2, false, false) + combinationsPeriodicBC(N - 2, false, true);
        } else {
            // 00..0: 00, 10, 01, 11 allowed:
            return 2*combinationsPeriodicBC(N - 2, false, false) + combinationsPeriodicBC(N - 2, false, true) + combinationsPeriodicBC(N - 2, true, true);
        }
    }
...
int main(){
    ...
    size_t result = solution.combinationsPeriodicBC(testN, false, false);
